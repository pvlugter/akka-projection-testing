...
object Generator {
  def apply(
    ...
    val random = RandomProvider(settings.random).expand(stageId, generatorId)
    val activityGenerator =
      ActivityGenerator(stageId, generatorId, start, duration, settings.entityId, settings.activity, random)
