...
object Runner {
  ...
  def run(id: String, settings: SimulationSettings, process: Event => Future[Done], log: Logger)(implicit
    ...
    ticks
      ...
      .mapAsyncUnordered(Engine.Settings.parallelism(settings.engine)) { event =>
        process(event).map(_ => event)(ExecutionContext.parasitic)
      }
      .runFold(Summary.Empty) { case (Summary(totalActivities, totalEvents), event) =>
        val newTotalActivities = if (event.seqNr == 1) totalActivities + 1 else totalActivities
        val newTotalEvents = totalEvents + 1
        if (newTotalEvents % 1000 == 0) {
